...
from agreement.synoptic_table import SynopticTable, ParallelTuple
import tests.config
...
def run_example_new(table_title, passages, report_filename, **kwargs):
    synopsis_model = SynopticTableModel(table_title, passages)
    synopsis_model.process_synopsis()

    console = Console(record=True)
    table_view = SynopticTableRichText(synopsis_model, color_scheme=GoodacreColorScheme(0, 1, 2))
